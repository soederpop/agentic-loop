...
turns: 36
toolCalls: 61
completedAt: '2026-03-22T23:28:11.188Z'
---
...
- Original idea: `docs/ideas/workflows/blank-slate-onboarding.md`
- Existing capture workflow pattern for `luca.serve.ts` structure

## Verification

- Running `luca serve` in `workflows/blank-slate/` starts on port 9301
- A user with no vision/goals sees step 1; a user with existing vision starts at step 2
- Completing the wizard creates real docs in `docs/goals/` and `docs/ideas/`
- `cnotes validate` passes on all created documents
