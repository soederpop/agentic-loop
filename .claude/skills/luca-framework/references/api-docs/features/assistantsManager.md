...
|------|------|----------|-------------|
| `name` | `string` | ✓ | The assistant name (e.g. 'assistants/chiefOfStaff') |
...
```ts
const assistant = manager.create('assistants/chiefOfStaff', { model: 'gpt-4.1' })
```
...
manager.discover()
console.log(manager.list()) // [{ name: 'assistants/chiefOfStaff', folder: '...', ... }]
const assistant = manager.create('assistants/chiefOfStaff')
